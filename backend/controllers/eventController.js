...
export const getUpcomingEventsWithPhoneOrder = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT e.*, TO_JSONB(o.*) AS orderUser FROM events e LEFT JOIN orders o ON e.id = o.event_id  AND o.phone = $1 WHERE date >= CURRENT_DATE  AND e.deleted = FALSE HAVING o.deleted = FALSE ORDER BY date ASC",
      [phone]
    );
    res.status(200).json(result.rows);
  } catch (err) {
